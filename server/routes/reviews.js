...
recordRoutes.route("/review").get(function (req, res) {
 Review.find({}).lean().exec((err, result) => {
     if (err) throw err;
     res.json(result)
 })
});
...
recordRoutes.route("/review/add").post(function (req, response) {
 const dbReview = new Review(req.body)
...
recordRoutes.route("/review/update/:id").post(function (req, response) {
 Review.findOneAndUpdate({_id: ObjectId(req.params.id)}, {$set: req.body}, (err, res) => {
     if (err) throw err;
